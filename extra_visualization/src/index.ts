...
async function torque_query(from?: string, until?: string, limit: number = 5000): Promise<any[]>
{
    const query = `
        SELECT
            s.tag,
...
async function position_query():Promise<any[]>
{
    const query = `
...
async function redline_query(from?: string,until?: string,limit: number = 100):Promise<any[]>
{
    const query = `
        WITH torque_data AS (
            SELECT
...
async function torque_redline_query(): Promise<any[]>
{
    const query = `
        SELECT
            s.tag,
...
async function status_query(from?: string, until?: string): Promise<any[]>
{
    const query = `
        SELECT
            "tagSetupUID",
...
async function sql_query(from?: string, until?: string,limit: number = 100): Promise<any[]>
{
    const query = `
        SELECT
            tag,
...
{ 
	...
	apiRouter.get('/redline', async (req: Request, res: Response) => {
		try {
			const limit = parseInt(req.query.limit as string) || 100;
			...
			const rows = await redline_query(
				from,
				until,
				limit
			);
    ...
    apiRouter.get('/torque/redline', async (req: Request, res: Response) => {
        try {
    ...
    apiRouter.get('/position', async (req: Request, res: Response) => {
        try {
